...
} cmdtab[] =
{
    ...
    // MMC_WRITE_BLOCK (R1)
    {24,            (24<< 24)|MMCCMDXTYPE_CICEN|MMCCMDXTYPE_CCCEN|MMCCMDXTYPE_RSPTYPL48|MMCCMDXTYPE_DPSEL },
    // MMC_WRITE_MULTIPLE_BLOCK (R1)
    {25,            (25<< 24)|MMCCMDXTYPE_CICEN|MMCCMDXTYPE_CCCEN|MMCCMDXTYPE_RSPTYPL48|MMCCMDXTYPE_DPSEL},
    // MMC_APP_CMD (R1)
...
static int mx_send_cmd(sdmmc_t *sdmmc)
{
    ...
    }else if (command->cmd == MMC_WRITE_BLOCK) {
        mix_ctrl = (MMCMIXCTRL_DMAEN);
    } else if (command->cmd == MMC_WRITE_MULTIPLE_BLOCK) {
        mix_ctrl = (MMCMIXCTRL_MSBSEL | MMCMIXCTRL_BCEN | MMCMIXCTRL_AC12EN | MMCMIXCTRL_DMAEN);
    }

    if (data_txf) {
        mx6x_setup_dma(sdmmc);
    }
     ...
     */
    out32(base + MX_MMCWATML, (0x80 << MMCWATML_RDWMLSHIFT)|(0x80 << MMCWATML_WRWMLSHIFT));
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/ipl/lib/sdmmc/mx_sdmmc.c $ $Rev: 795041 $")
#endif
