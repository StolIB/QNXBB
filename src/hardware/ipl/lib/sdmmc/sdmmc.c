...
static void mmc_parse_ext_csd(sdmmc_t *sdmmc, unsigned char *raw_ecsd)
{
	...
	if (sdmmc->verbose) {
		...
		ser_putstr("	SEC "); ser_putdec(card->blk_num); ser_putstr("\n");
	}
}
...
static int mmc_send_op_cond(sdmmc_t *sdmmc)
{
	...
	CMD_CREATE (sdmmc->cmd, MMC_SEND_OP_COND, OCR_HCS | sdmmc->ocr, rsp, 0, 0, 0);
...
static int mmc_init_hs(sdmmc_t *sdmmc)
{
	...
	if (mmc_switch(sdmmc, MMC_SWITCH_MODE_WRITE, ECSD_BUS_WIDTH, (sdmmc->bus_width == 8)? ECSD_BUS_WIDTH_8_BIT : ECSD_BUS_WIDTH_4_BIT, MMC_SWITCH_CMDSET_DFLT)) {
		ser_putstr("Can't switch to 8 bit mode\n");
		return SDMMC_ERROR;
	}

	hc->set_bus_width(sdmmc, sdmmc->bus_width);
...
static int mmc_init_ddr(sdmmc_t *sdmmc)
{
	...
	if (mmc_switch(sdmmc, MMC_SWITCH_MODE_WRITE, ECSD_BUS_WIDTH, (sdmmc->bus_width == 8)? ECSD_BUS_WIDTH_DDR_8_BIT : ECSD_BUS_WIDTH_DDR_4_BIT, MMC_SWITCH_CMDSET_DFLT)) {
		ser_putstr("Can't switch to 8 bit mode\n");
		return SDMMC_ERROR;
	}
...
int sdmmc_select_bt_part_access(sdmmc_t *sdmmc, char bt_part) {

        if ( mmc_switch(sdmmc, MMC_SWITCH_MODE_SET, ECSD_PART_CONFIG, bt_part, MMC_SWITCH_MODE_CMD_SET) != SDMMC_OK ) {
		ser_putstr(" sdmmc_select_bt_part() failed\n");
		return SDMMC_ERROR;
	}
	return SDMMC_OK;
}
...
static int _sdmmc_read_write(void *ext, void *buf, unsigned blkno, unsigned blkcnt, int read){
	...
	while (1) {
		...
		if (cblkcnt == 1) {
			cmd = (read)?MMC_READ_SINGLE_BLOCK:MMC_WRITE_BLOCK;
		} else {
			cmd = (read)?MMC_READ_MULTIPLE_BLOCK:MMC_WRITE_MULTIPLE_BLOCK;
		}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/ipl/lib/sdmmc/sdmmc.c $ $Rev: 795494 $")
#endif
