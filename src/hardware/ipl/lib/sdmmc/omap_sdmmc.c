...
#include <hw/inout.h>
extern int omap_reset( sdmmc_t *sdmmc, unsigned rst);
...
} cmdtab[] =
{
	...
	// MMC_WRITE_BLOCK (R1)
	{ 24,			(24 << 24) | CMD_CICE | CMD_CCCE | CMD_RSP_TYPE_48 | CMD_DP | CMD_DE },
	// MMC_WRITE_MULTIPLE_BLOCK (R1)
	{ 25,			(25 << 24) | CMD_CICE | CMD_CCCE | CMD_RSP_TYPE_48 | CMD_DP | CMD_DE | CMD_MBS | CMD_BCE | CMD_ACEN },
	// MMC_TUNING
	{ 21,			(21 << 24) | CMD_CICE | CMD_CCCE | CMD_RSP_TYPE_48 | CMD_DP | CMD_DDIR },
	// MMC_APP_CMD (R1)
...
static void omap_set_timing(sdmmc_t *sdmmc, int timing)
{
	...
	sdmmc_hc_t	*hc = sdmmc->hc;
	unsigned	base = hc->sdmmc_pbase;
	unsigned	con, hctl, hctl2;

	if (sdmmc->verbose > SDMMC_VERBOSE_LVL_0) {
		ser_putstr("omap_set_timing ");
		ser_putstr(name[timing]);
		ser_putstr("\n");
	}

	con		= in32( base + OMAP_MMCHS_CON ) & ~CON_DDR;
	hctl	= in32( base + OMAP_MMCHS_HCTL ) & ~HCTL_HSE;
	hctl2	= 0;

	if( hc->version >= REV_SREV_V3 ) {
		hctl2	= in32( base + OMAP_MMCHS_HCTL2 ) & ~HCTL2_MODE_MSK;
	}
	...
	if( hc->version >= REV_SREV_V3 ) {
		...
		if( ( hctl2 & HCTL2_PRESET_VAL ) ) {
			out32( base + OMAP_MMCHS_SYSCTL, in32( base + OMAP_MMCHS_SYSCTL ) & ~SYSCTL_CEN );
		}
...
static int omap_init_ctrl_v33(sdmmc_t *sdmmc)
{
	unsigned base = sdmmc->hc->sdmmc_pbase;
...
{
	unsigned base = sdmmc->hc->sdmmc_pbase;
	sdmmc->hc->version = REV_SREV( in32( base + OMAP_MMCHS_REV ) );
	if((sdmmc->caps & SDMMC_CAP_HS200)){
		return omap_init_ctrl_v18(sdmmc);
	} else {
		return omap_init_ctrl_v33(sdmmc);
	}
}
...
int omap_sdmmc_fini(sdmmc_t *sdmmc)
{
	...
	out32(base + OMAP_MMCHS_IE, 0);
	out32(base + OMAP_MMCHS_STAT, INTR_ALL);
...
static int omap_pio_read(sdmmc_t *sdmmc, void *buf, unsigned len)
{
	...
	unsigned	imask = INTR_BRR | INTR_ERRI;
	 ...
	 *
	 * Here the same driver, tuning read does not work on J6.
	 */
	if(!(sdmmc->caps & SDMMC_CAP_HS200)) {
		if(omap_wait_bits(sdmmc, OMAP_MMCHS_PSTATE, (PSTATE_CMDI | PSTATE_DATI), 0)) return SDMMC_ERROR;
	}
	...
	/* clear SDMMC status */
	out32(base + OMAP_MMCHS_STAT, STAT_CLR_MSK);
	...
	do {
		omap_wait_bits(sdmmc, OMAP_MMCHS_STAT, imask, imask);
		stat = in32(base + OMAP_MMCHS_STAT);
		if (cmd->cmd == MMC_SEND_TUNING_BLOCK && (!(stat & imask) || stat & INTR_ERRI)) {
			if(sdmmc->verbose > SDMMC_VERBOSE_LVL_0) {
				ser_putstr("omap_pio_read fialed MMCHS_STAT 0x");
				ser_puthex(stat);
				ser_putstr("\n");
			}
			out32(base + OMAP_MMCHS_STAT, stat);
			omap_reset( sdmmc, SYSCTL_SRD );
			omap_reset( sdmmc, SYSCTL_SRC );
			return SDMMC_ERROR;
		}
	...
	/*
	 * check error status
	 * HS200 failed sometimes, keep the same as driver
	 */
	if(cmd->cmd != MMC_SEND_TUNING_BLOCK) {
		if (in32(base + OMAP_MMCHS_STAT) & INTR_ERRI) {
			if (sdmmc->card.state != MMC_IDLE) {
...
static int omap_setup_adma(sdmmc_t *sdmmc)
{
...
static void omap_sdmmc_adma(sdmmc_t *sdmmc, void *dma_ext)
{
	...
	sdmmc->hc->setup_dma = omap_setup_adma;
...
static sdmmc_hc_t omap_hc = {
	.set_clk = omap_set_frq,
	.set_bus_width = omap_set_bus_width,
	.set_bus_mode = omap_set_bus_mode,
	.set_timing = omap_set_timing,
	.send_cmd = omap_send_cmd,
	.pio_read = omap_pio_read,
	.init_hc = omap_init_ctrl,
	.dinit_hc = omap_sdmmc_fini,
	.signal_voltage = omap_signal_voltage,
	.pwr = omap_pwr,
	.tuning = omap_tune,
};
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/ipl/lib/sdmmc/omap_sdmmc.c $ $Rev: 795041 $")
#endif
