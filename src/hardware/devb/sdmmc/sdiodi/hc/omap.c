...
static int omap_timing( sdio_hc_t *hc, int timing )
{
	...
	switch( timing ) {
		case TIMING_HS200:
			...
			if ( ( hc->cfg.did == OMAP_DID_54XX ) || ( hc->cfg.did == OMAP_DID_54XX_ES2 ) || ( hc->cfg.did == OMAP_DID_DRA7XX ) )
...
static int omap_signal_voltage( sdio_hc_t *hc, int signal_voltage )
{
	...
	if( signal_voltage == SIGNAL_VOLTAGE_1_8 ) {
		...
		if( ( cfg->did == OMAP_DID_54XX ) || ( cfg->did == OMAP_DID_54XX_ES2 ) || ( cfg->did == OMAP_DID_DRA7XX ) ||
...
int omap_init( sdio_hc_t *hc )
{
	...
	if ( ( cfg->did == OMAP_DID_54XX_ES2 ) || ( cfg->did == OMAP_DID_DRA7XX ) )
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devb/sdmmc/sdiodi/hc/omap.c $ $Rev: 779664 $")
#endif
