...
static int generic_cd( sdio_hc_t *hc )
{
	...
	status = in32(base + MMCHS_PSTATE);

	if (status & PSTATE_CINS) {
		cstate |= CD_INS;

		/* Write protected */
		if ((status & PSTATE_WP) && !ext->nowp){
			cstate |= CD_WP;
		}
		else{
			cstate &= ~CD_WP;
...
static int generic_gpio_cd(sdio_hc_t *hc)
{
	...
	if (!(in32(ext->cd_base + GPIO_DATAIN) & (1 << ext->cd_pin))) {
		...
		/* Write protected */
		if ((ext->wp_base != MAP_DEVICE_FAILED) && !(in32(ext->wp_base + GPIO_DATAIN) & (1 << ext->wp_pin))){
			cstate |= CD_WP;
		}
		else {
			cstate &= ~CD_WP;
 ...
 * For example:
 *		cd=0x48032000^27^795 is equavalent to cd_base=0x48032000:cd_pin=27:cd_irq=795
 *		wp=0x48032000^27 is equavalent to wp_base=0x48032000:wp_pin=27
 */
...
#define	NOWP		2
			"nowp",		/* no write protection capability */
#define	CD_IRQ		3
...
#define	CD_PIN		5
			"cd_pin",
#define CD			6	/* This option covers CD_IRQ, CD_BASE, CD_PIN */
			"cd",
#define	WP_BASE		7
			"wp_base",	/* GPIO base address for the WP pin */
#define	WP_PIN		8
			"wp_pin",
#define WP			9	/* This option covers WP_BASE, WP_PIN */
			"wp",
#define	BW			10
			"bw",		/* data bus width */
#define	VDD1_8		11
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devb/sdmmc/arm/j5_generic.le.v7/bs.c $ $Rev: 784273 $")
#endif
