...
static int mcasp_fifo_threshold(uint8_t serializer_cnt, ado_pcm_config_t *config)
{
	/* J6 Erratum: APCA-011 – McASP to EDMA Synchronization Level Event can be lost
	 *             Increase the time between when McASP EDMA event is set and cleared by increasing
	 *             the FIFO threshold. Bound the threshold between 16 and 32 words to workaround this
	 *             errata and to prevent overrun/underrun.
	 *
	 *             NOTE: The fragment size (DMA transfer size) must be an even multiple of the threshold
	 *                   and the threshold must be evenly divisable by the number of active serializers.
	 */
#define MIN_FIFO_THRESHOLD 16
#define MAX_FIFO_THRESHOLD (WFIFOCTL_WC / 2)
	int fifo_threshold;
	for (fifo_threshold = MAX_FIFO_THRESHOLD; fifo_threshold >= MIN_FIFO_THRESHOLD; fifo_threshold--)
	{
		if (fifo_threshold % serializer_cnt == 0)
		{
			if (config->mode.block.frag_size % fifo_threshold == 0)
				break;	/* We found a suitable threshold */
		}
	}
	if (fifo_threshold < MIN_FIFO_THRESHOLD)
	{
		fifo_threshold = MAX_FIFO_THRESHOLD / serializer_cnt * serializer_cnt;
		ado_error("%s: WARNING - Suitable FIFO threshold could not be found, defaulting to %d words", __FUNCTION__, fifo_threshold);
	}

	/* Fragment size has to be an even multiple of the FIFO threshold
	 * Note: Just check the LSB to see if frag_size is an even multiple of the FIFO threshold or not.
	 */
	if ((config->mode.block.frag_size % fifo_threshold != 0) || ((config->mode.block.frag_size / fifo_threshold) & 0x1))
	{
		int frame_size = ado_pcm_sample_bytes(config) * serializer_cnt;
		int alignment = ado_lcm(fifo_threshold * 2, frame_size);	/* Multiple by 2 to make it an even multiple */

		ado_error("%s: WARNING - Aligning fragment (%d) to be an even multiple of the FIFO threshold (%d) and the frame size (%d) - Alignment = %d",
			__FUNCTION__, config->mode.block.frag_size, fifo_threshold, frame_size, alignment);
	...
	ado_debug(DB_LVL_DRIVER, "%s: FIFO Threshold = %d, serializer count = %d, Fragment size = %d", __FUNCTION__, fifo_threshold, serializer_cnt, config->mode.block.frag_size);
...
int32_t mcasp_capabilities(HW_CONTEXT_T * mcasp_card, ado_pcm_t *pcm, snd_pcm_channel_info_t * info)
{
	...
	if (info->channel == SND_PCM_CHANNEL_PLAYBACK)
	{
		if (mcasp_card->play_strm.serializer_cnt > 1)
			info->fragment_align = mcasp_card->play_strm.serializer_cnt * mcasp_card->sample_size;
	...
	else if (info->channel == SND_PCM_CHANNEL_CAPTURE)
	{
		...
		/* If multiple serializers or subchns are used, align the subchn fragment size with the underlying DMA transfer size
		 * NOTE: We do this to ensure the unblock interval on the client side is consistent with the data rate of the hardware
		 */
		if (mcasp_card->cap_aif.serializer_cnt > 1 || cap_strm->nsubchn > 1)
			info->fragment_align = mcasp_card->cap_aif.cap_strm[0].pcm_caps.min_fragsize;
...
void set_play_bit_clock_rate(HW_CONTEXT_T *mcasp_card, unsigned rate)
{
	...
	/* If not in asynchronous clock mode then we must
	 * lock the capture sample rate to the playback sample rate.
	 */
					  ...
					  ado_pcm_config_t * config, ado_pcm_subchn_t * subchn, uint32_t * why_failed)
{
	...
	config->dmabuf.flags = ADO_SHM_DMA_SAFE | ADO_BUF_CACHE;
	...
	mcasp_card->play_strm.fifo_thres = mcasp_fifo_threshold(mcasp_card->play_strm.serializer_cnt, config);
	...
	mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].BCNT = mcasp_card->play_strm.fifo_thres;
	...
	mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].SRCCIDX = mcasp_card->sample_size * mcasp_card->play_strm.fifo_thres;
	mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].DSTCIDX = 0;
	mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].CCNT =
		ado_pcm_dma_int_size(config) / (mcasp_card->sample_size * mcasp_card->play_strm.fifo_thres);	/* Number of frame/time_slot per param set */
					 ...
					 ado_pcm_config_t * config, ado_pcm_subchn_t * subchn, uint32_t * why_failed)
{
	...
	/* If serializer_cnt and nsubchn == 1, then we DMA directly into the pcm buffer, so make it DMA safe */
	...
	if(0 == opened_subchns)
	{
		...
		if (mcasp_card->cap_aif.serializer_cnt == 1 && strm->nsubchn == 1)
		{
			...
			mcasp_card->cap_aif.fifo_thres = mcasp_fifo_threshold(mcasp_card->cap_aif.serializer_cnt, config);
			/* Update cap_aif.frag_size with latest config fragsize */
		...
		mcasp_card->edma3->PaRAM[strm->dma_idx].BCNT = mcasp_card->cap_aif.fifo_thres;
		...
		mcasp_card->edma3->PaRAM[strm->dma_idx].DSTCIDX = mcasp_card->sample_size * mcasp_card->cap_aif.fifo_thres;
		mcasp_card->edma3->PaRAM[strm->dma_idx].CCNT =
			mcasp_card->cap_aif.frag_size / (mcasp_card->sample_size * mcasp_card->cap_aif.fifo_thres);    /* Number of frame/time_slot per param set */
...
int32_t mcasp_prepare(HW_CONTEXT_T * mcasp_card, PCM_SUBCHN_CONTEXT_T * subchn, ado_pcm_config_t * config)
{
	...
	if (subchn->strm == &mcasp_card->play_strm)
	{
		...
		mcasp_card->edma3->PaRAM[strm->dma_idx].CCNT =
			(ado_pcm_dma_int_size(config) / (mcasp_card->sample_size * mcasp_card->play_strm.fifo_thres));
	}
	else
	{
		...
		if(mcasp_card->cap_aif.active == 0)
		{
			...
			/* Reset the CCNT (Frames per param set) */
			mcasp_card->edma3->PaRAM[strm->dma_idx].CCNT =
				(mcasp_card->cap_aif.frag_size / (mcasp_card->sample_size * mcasp_card->cap_aif.fifo_thres));
		}
		ado_mutex_unlock(&mcasp_card->hw_lock);
...
int32_t mcasp_trigger(HW_CONTEXT_T * mcasp_card, PCM_SUBCHN_CONTEXT_T * pc, uint32_t cmd)
{
	...
	if (cmd == ADO_PCM_TRIGGER_GO)
	{
		...
		if (pc == &mcasp_card->play_strm.subchn)
		{
			 ...
			 */
			mcasp_card->mcasp->WFIFOCTL = WFIFOCTL_WNUMEVT(mcasp_card->play_strm.fifo_thres) | WFIFOCTL_WNUMDMA(mcasp_card->play_strm.serializer_cnt);
			 ...
			 * NOTE: The full level will be a multiple of the fifo threshold minus the number of active serializers
			 */
			fifo_full_lvl = (WFIFOCTL_WC / mcasp_card->play_strm.fifo_thres * mcasp_card->play_strm.fifo_thres) - mcasp_card->play_strm.serializer_cnt;
			timeout = 1000;
			while (((mcasp_card->mcasp->WFIFOSTS & WFIFOSTS_WLVL) < fifo_full_lvl) && timeout--)
				nanospin_ns(100);
			if (timeout <= 0)
				ado_error("mcasp_trigger: AFIFO is not full, %d", mcasp_card->mcasp->WFIFOSTS & WFIFOSTS_WLVL);
#endif
		...
		else
		{
			if (mcasp_card->cap_aif.active == 0)
			{
				 ...
				 */
				mcasp_card->mcasp->RFIFOCTL = RFIFOCTL_RNUMEVT(mcasp_card->cap_aif.fifo_thres) | RFIFOCTL_RNUMDMA(mcasp_card->cap_aif.serializer_cnt);
...
{
	int32_t nbytes = 0, frag_size = 0;

	if (mcasp_card->play_strm.subchn.pcm_config != NULL) {
		frag_size = ado_pcm_dma_int_size(mcasp_card->play_strm.subchn.pcm_config);
		nbytes = frag_size - (mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].CCNT * mcasp_card->edma3->PaRAM[mcasp_card->play_strm.dma_idx].SRCCIDX);
	}

#if !defined(VARIANT_j2_pcm3168a)
	ado_error("mcasp_card: TX Underrun occured! - TX AFIFO count = %d, DMA position = %d/%d,", mcasp_card->mcasp->WFIFOSTS & WFIFOSTS_WLVL,
				nbytes, frag_size);
#else
	ado_error("mcasp_card: TX Underrun occured! - DMA Position = %d/%d", nbytes, frag_size);
#endif
	mcasp_card->mcasp->XSTAT |= XSTAT_XUNDRN;
}
...
subchn_dmacapture(HW_CONTEXT_T *mcasp_card, uint8_t *srcDMAAddr, uint32_t size)
{
	...
	for(idx = 0; idx < mcasp_card->cap_aif.cap_strm[0].nsubchn; idx++)
	{
		...
		if((NULL != ctx->pcm_subchn) && (1 == ctx->go))
		{
			...
			{
				/* We only want to process up to one fragment at a time, so limit the memcpy() to either the
				 * remaining bytes of a paritially copied fragment, or to one full fragment. If the overall
				 * size is greater then a fragment then additional iterations through the loop will handle it.
				 */
				remaining = ado_pcm_dma_int_size(config) - (ctx->pcm_offset % ado_pcm_dma_int_size(config));
				bytesToTransfer = MIN(remaining, (size - bytesTransferred));

				memcpy((uint8_t *)(config->dmabuf.addr + ctx->pcm_offset), &srcDMAAddr[bytesTransferred], bytesToTransfer);
				/* Update pcm_offset and account for buffer wrap */
...
serializer_dmacapture(HW_CONTEXT_T *mcasp_card, uint8_t *srcDMAAddr, uint32_t size)
{
	...
	/* Invalidate cache for local DMA buffer */
...
mcasp_parse_commandline(HW_CONTEXT_T * mcasp_card, char *args)
{
...
#if defined (DEFAULT_MCASP)
	mcasp_card->mcasp_idx = DEFAULT_MCASP;
#else
...
#if defined (CODEC_I2C_DEV)
	mcasp_card->i2c_dev = CODEC_I2C_DEV;
#else
	...
	while (args != NULL && args[0] != 0)
	{
		switch ((opt = getsubopt(&args, opts, &value)))
		{
			...
			case 2:
				if (value != NULL)
				{
					int mcasp_idx = strtoul(value, NULL, 0);

#if defined (VARIANT_J6)
					/* McASP indices on the J6 are from 1->8 */
					if (mcasp_idx > 0 && mcasp_idx < 9)
#else
					if (mcasp_idx >= 0 && mcasp_idx < 6)
#endif
					{
						mcasp_card->mcasp_idx = mcasp_idx;
						ado_debug(DB_LVL_DRIVER, "mcasp_card: Using MCASP%d", mcasp_card->mcasp_idx);
					}
					else
						ado_debug(DB_LVL_DRIVER, "mcasp_card: Invalid MCASP index, using default index %d", mcasp_card->mcasp_idx);
				}
				break;
			case 3:
				if (value != NULL)
	...
	/* Note: The J6 MCASP indices are not zero based, so
	 *       subtract 1 to align with the driver's zereo based indices.
	 */
#if defined (VARIANT_J6)
	switch (mcasp_card->mcasp_idx - 1)
#else
	...
	if (mcasp_card->cap_aif.serializer_cnt > 1 || mcasp_card->cap_aif.cap_strm[0].nsubchn > 1)
	{
		...
		config.mode.block.frag_size = mcasp_card->cap_aif.frag_size = rx_fragsize;
		config.format.voices = mcasp_card->cap_aif.cap_strm[0].voices;
		/* We only support 16 and 32 samples */
		config.format.format = mcasp_card->sample_size == 2 ? SND_PCM_SFMT_S16_LE : SND_PCM_SFMT_S32_LE;
		mcasp_card->cap_aif.fifo_thres = mcasp_fifo_threshold(mcasp_card->cap_aif.serializer_cnt, &config);
		/* Update cap_aif.frag_size with latest config frag_size */
		mcasp_card->cap_aif.frag_size = config.mode.block.frag_size;
	}
...
mcasp_playback_reconstitute16 (HW_CONTEXT_T * mcasp_card, PCM_SUBCHN_CONTEXT_T * pc, int8_t * dmaptr, size_t size)
{
	...
	for (i = 0; i < (size / sizeof (int16_t)); i+=(mcasp_card->play_strm.voices * mcasp_card->play_strm.serializer_cnt))
	{
		...
		{
			for(src_idx = iter; src_idx < (mcasp_card->play_strm.voices * mcasp_card->play_strm.serializer_cnt); src_idx+=mcasp_card->play_strm.voices, dst_idx++)
...
void mcasp_init(mcasp_card_t * mcasp_card)
{
	...
	else
	{
		...
		else if (mcasp_card->slot_num == 16)
		{
			mcasp_card->mcasp->AFSXCTL = AFSXCTL_XMOD_16CH;	// FS period = 16 words
			mcasp_card->mcasp->AFSRCTL = AFSRCTL_RMOD_16CH;
		}
		else
		{
...
int ctrl_init(HW_CONTEXT_T ** hw_context, ado_card_t * card, char *args)
{
	...
	*hw_context = mcasp_card;
	...
	if (mcasp_card->cap_aif.serializer_cnt > 1 || mcasp_card->cap_aif.cap_strm[0].nsubchn > 1)
	{
		...
		mcasp_card->cap_aif.dmabuf->flags = ADO_SHM_DMA_SAFE | ADO_BUF_CACHE;
		if((mcasp_card->cap_aif.dmabuf->addr = ado_shm_alloc(mcasp_card->cap_aif.dmabuf->size, mcasp_card->cap_aif.dmabuf->name, mcasp_card->cap_aif.dmabuf->flags, &mcasp_card->cap_aif.dmabuf->phys_addr)) == NULL)
		{
	...
	/* When serializer_cnt or nsubchns > 1 a separate DMA buffer will be used.
	 * The underlying DMA transfer will be split over the number of active rx serializers (virtual device buffers),
	 * So ensure the subchn min_fragsize is set to the size of the DMA transfer divided by the number of serializers 
	 * (i.e. subchn fragement size can be >= to the underlying DMA transfer size divided by the number of virtual device buffers)
	 */
	if (mcasp_card->cap_aif.serializer_cnt > 1 || mcasp_card->cap_aif.cap_strm[0].nsubchn > 1)
		mcasp_card->cap_aif.cap_strm[0].pcm_caps.min_fragsize = (mcasp_card->cap_aif.frag_size / mcasp_card->cap_aif.serializer_cnt);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/deva/ctrl/mcasp/mcasp_dll.c $ $Rev: 796667 $")
#endif
