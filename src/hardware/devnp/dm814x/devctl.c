	...
	for (i = RXGOODFRAMES; i <= RXDMAOVERRUNS; i += 4) {
		...
		switch (i) {
			...
			case	TXCARRIERSENSEERRORS:
				if (!(ti814x_is_br_phy (ti814x)))	/* BroadReach PHY always indicates no carrier */
	...
	switch (cmd) {
		...
		case	SIOCGIFMEDIA: {
			...
			if (ti814x_is_br_phy (ti814x)) {
				...
				if ((ti814x->cfg.flags & NIC_FLAG_LINK_DOWN) == 0) {
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/dev_qnx/dm814x/devctl.c $ $Rev: 791963 $")
#endif
