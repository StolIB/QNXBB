...

{
	...
	const struct sigevent		*ev, *rc = NULL;
	...
	if (misc_stat & STATPEND) {
	    ...
	    val &= ~STAT_PEND_EN;
	    out32(attach_args->cpsw_base + C0_MISC_EN, val);
	    ev = interrupt_queue(attach_args->iopkt, stat_ent);
	    if (ev != NULL) {
		rc = ev;
	    }
	}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/dev_qnx/dm814x/event.c $ $Rev: 788816 $")
#endif
