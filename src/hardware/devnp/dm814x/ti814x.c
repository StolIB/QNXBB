...
#define DM814OPT_P1RXFLOW	24
	"ptpmux",
#define DM814OPT_PTPMUX		25
	"p0master",
#define	DM814OPT_P0MAST		26
	"p1master",
#define	DM814OPT_P1MAST		27
	NULL
};
...
void ti814x_parse_options(ti814x_dev_t *ti814x, const char *optstring,
			  nic_config_t *cfg)

{
	...
	case DM814OPT_P1RXFLOW:
	    ...
	    break;
#endif
	case DM814OPT_PTPMUX:
	    if (ti814x != NULL) {
		    ti814x->ptp_enable = strtoul(value, 0, 0) + 1;
	    }
	    break;
	case DM814OPT_P0MAST:
		if (ti814x != NULL) {
			if (value) {
				ti814x->brmast = strtoul (value, 0, 0);
			}
		else {
			ti814x->brmast = 1;
		}
...
void ti814x_stop (struct ifnet *ifp, int disable)
{
    ...
    MDI_PowerdownPhy(ti814x->mdi, ti814x->cfg.phy_addr);
...
static void ti814x_clean_one_port (ti814x_dev_t *ti814x, int clean_rx)
...

{
    ...
    ti814x_clean_one_port(ti814x, 1);
...

{
    ...
    ti814x_parse_options(NULL, options, &attach_args.cfg);
...

{
    ...
    /* Parse options */
    ti814x_parse_options(ti814x, options, &ti814x->cfg);
...
cleanup:
    ti814x_clean_one_port(ti814x, clean_rx);

    /* Only clean the common stuff if this is the first interface */
    if (ti814x->get_stats) {
	ti814x_detach_cleanup(attach_args, -1);
    }
    return err;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/lib/io-pkt/sys/dev_qnx/dm814x/ti814x.c $ $Rev: 789066 $")
#endif
