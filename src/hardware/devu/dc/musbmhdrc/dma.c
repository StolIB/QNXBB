...
static int create_dma_interrupt_handler( dma_ctx_t * dma_ctx ) {
	...
	dma_ctx->dma_event.sigev_priority        = dc->priority;
...
void dma_transfer_start( void * hdl, int chidx, uint32_t start_addr_paddr, unsigned length, dmamode_t dmamode ) {
	...
	if ( ch_ctx->ep->dir == USB_ENDPOINT_IN  ) {
		uint16_t        txcsr = HW_Read16( dc->IoBase, MUSB_TXCSR(ch_ctx->ep->num ) );
		if ( ( txcsr & TXCSR_DMA_REQ_EN ) == 0 ) {
			...
			*/
			...
			HW_Write16( dc->IoBase, MUSB_TXCSR(ch_ctx->ep->num ), txcsr | TXCSR_DMA_REQ_EN );
		}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devu/dc/musbmhdrc/dma.c $ $Rev: 776862 $")
#endif
