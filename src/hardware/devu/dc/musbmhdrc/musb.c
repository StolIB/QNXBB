...
void complete_urb( dctrl_t * dc, ep_ctx_t *ep, uint32_t urb_status ) {
	...
	if ( (td = SIMPLEQ_FIRST( &ep->transfer_q )) && (urb = td->urb)) {
		...
		next_td = NULL;
		if( urb_status == EOK && ep->type != USB_ATTRIB_ISOCHRONOUS && ep->num != 0 ){
			//for non control transfer try start the next one if available
			if ( (next_td = SIMPLEQ_FIRST( &ep->transfer_q ))) {
				ep->td = next_td;
				if ( ep->dir & USB_ENDPOINT_IN) {
					MUSB_UNLOCK;
					ep_transmit( dc, ep );
...
#else			
		SIMPLEQ_INSERT_TAIL( &dc->transfer_free_q, td, link );
		MUSB_SLOGF_DBG(dc, _SLOG_DEBUG1, "URB COMPLETE epnum = 0x%x epdir = 0x%x urb->actual_len = %d status=%x",ep->num, ep->dir , urb->actual_len, urb_status  );
...
void ep_transmit( dctrl_t *dc, ep_ctx_t 	*ep ) {
	musb_transfer_t *td = ep->td;
	uint32_t		len;
			
	if( td == NULL || td->xfer_length < td->bytes_xfered){
		return ;
	}
	
	if ( USE_BUILTIN_DMA( dc, ep ) && ( td->xfer_length > td->bytes_xfered ) ) {
		dc->dma_funcs.transfer_start( dc->dma_hdl, ep->dma_chidx, td->xfer_buffer_paddr + td->bytes_xfered, td->xfer_length - td->bytes_xfered, DMA_MODE1 );
	} else {
...
/*before we call this, make sure musb_locked */
void ep_receive( dctrl_t *dc, ep_ctx_t *ep ) {
	musb_transfer_t *td = ep->td;

	if( td == NULL ){
		MUSB_UNLOCK
		MUSB_SLOGF_ISR_DBG(dc, _SLOG_DEBUG1, "%s(%d): epnum = %d rxcsr = %x",__func__, __LINE__, ep->num, HW_Read16( dc->IoBase, MUSB_RXCSR( ep->num ) ) );		
		return ;
	}
...
static void process_epout_interrupt( dctrl_t *dc, unsigned epidx ) {
	...
	if(ep->type == USB_ATTRIB_ISOCHRONOUS){
		...
		if( (rxcsr & RXCSR_RXPKTRDY) && !(dc->flags & DC_FLAG_CPPI_DMA)) {
			td = ep->td;
			...
			case MUSB_PULSE_INTR:
				{
					...
					do {
						...
						if ( td != NULL ) {
							...
							if((td->flags & TD_STS_COMP_MSK) && dc->musb_completion_poll){
								dc->musb_completion_poll(dc, td);
								// it's up to the poll handler to complete the urb
								// and return the td to the free poll
								MUSB_LOCK;
							}
							else{
...
static ep_ctx_t 
*musb_endpoint_initialize( void *chdl, iousb_device_t *device, iousb_endpoint_t *iousb_ep )
{
	usbdc_device_t 	*udc = ( usbdc_device_t * ) chdl;
	dctrl_t 		*dc = ( dctrl_t * ) udc->dc_data;
	ep_ctx_t		*ep = iousb_ep->user; 
...
musb_endpoint_enable( void *chdl, iousb_device_t *device, iousb_endpoint_t *iousb_ep )
{
	...
	if( (ep = musb_endpoint_initialize( chdl, device, iousb_ep ) )== NULL){
		return EINVAL;
	}
	
	udc = ( usbdc_device_t * ) chdl;
	dc = ( dctrl_t * ) udc->dc_data;
		
	if( dc->flags & DC_FLAG_MURB ){
		iousb_ep->flags |= IOUSB_ENDPOINT_MURB;
	}

	// reset the toggle
	if ( ep->dir & USB_ENDPOINT_IN ) {
		HW_Write16( dc->IoBase, MUSB_TXCSR( ep->num ), TXCSR_CLR_DATA_TOGGLE );
		if ( USE_BUILTIN_DMA( dc, ep ) && !(dc->flags & DC_FLAG_CPPI_DMA)) {
		...
		HW_Write16( dc->IoBase, MUSB_RXCSR( ep->num ), RXCSR_FLUSHFIFO|RXCSR_CLR_DATA_TOGGLE );
		if ( USE_BUILTIN_DMA( dc, ep )  && !(dc->flags & DC_FLAG_CPPI_DMA) ) {
...
musb_transfer_abort( void *chdl, iousb_transfer_t *urb, iousb_endpoint_t *iousb_ep )
{
	...
	MUSB_SLOGF_DBG(dc, _SLOG_DEBUG1, "%s: ep = 0x%x  ep->num = %d ", __func__, iousb_ep->edesc.bEndpointAddress, ep->num);
	if(ep->td || (td = SIMPLEQ_FIRST( &ep->transfer_q )) != NULL)
		active =1;
	...
	if ( active && (USE_BUILTIN_DMA( dc, ep ) || USE_EXTERNAL_DMA( dc, ep ) )) {
...
musb_transfer( void *chdl, iousb_transfer_t *urb, iousb_endpoint_t *iousb_ep, uint8_t *buffer, _uint32 length, _uint32 flags )
{
	...
	MUSB_LOCK;
	if( SIMPLEQ_FIRST( &ep->transfer_q ) || (ep->flags & EP_FLAG_XFER_INPROGRESS) ){ 
		//not empty queue, simply enqueue it
		SIMPLEQ_INSERT_TAIL( &ep->transfer_q, td, link );
		MUSB_UNLOCK;
	}    
	else{
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devu/dc/musbmhdrc/musb.c $ $Rev: 776862 $")
#endif
