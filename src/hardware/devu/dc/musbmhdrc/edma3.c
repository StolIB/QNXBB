...
edma_intr_handler( void *area, int id )
{
	...
	if( ch_ctx->ep->dir == USB_ENDPOINT_IN ) {
		...
		if( ( td->bytes_xfered >= td->xfer_length) || ( ch_ctx->transfer_len != ch_ctx->ep->mps) ) {
			...
			// will trigger urb completion
			if ( SIMPLEQ_FIRST( &dc->transfer_complete_q) ) {
...
edma_init( dctrl_t * dc )
{
	...
	for( epnum = 1; epnum < edma_ctx->num_ep; epnum++ ) {
		...
		ch_ctx->edma_event.sigev_priority		= dc->priority;
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/6.6.0/trunk/hardware/devu/dc/musbmhdrc/edma3.c $ $Rev: 776862 $")
#endif
